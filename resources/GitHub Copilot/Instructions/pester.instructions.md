---
id: copilot-pester-instructions
title: PowerShell Pester v5 Testing Guidelines
summary: Testing guidelines for building reliable PowerShell Pester v5 test suites.
tool: GitHub Copilot
task_tags:
    - powershell
    - testing
...
Common assertion keywords for validating test outcomes:
...
- **Strings**: `-Match`, `-MatchExactly`, `-BeLike`, `-BeLikeExactly`, `-BeNullOrEmpty`
- **Types**: `-BeOfType`, `-BeTrue`, `-BeFalse`
- **Files**: `-Exist`, `-FileContentMatch`, `-FileContentMatchExactly`, `-FileContentMatchMultiline`, `-FileContentMatchMultilineExactly`
- **Exceptions**: `-Throw`, `-Not -Throw`

```powershell
# String validation (use -Match for pattern, -Be for exact match, or -BeLike for wild card pattern)
$string | Should -Match 'pattern'
$string | Should -BeLike 'wildcardvalue *'
...
# Collection item count validation
$array | Should -HaveCount 5

# Exception handling
{ Get-InvalidUser } | Should -Throw
```

### Custom Assertions

For complex validation logic not covered by built-in assertions, create a custom `Should` operator using `Add-AssertionOperator`:

```powershell
BeforeAll {
    Add-AssertionOperator -Name ValidPhoneFormat -Test {
        param($ActualValue)
        $isValid = $ActualValue -match '^\d{3}-\d{3}-\d{4}$'
        [PSCustomObject]@{
            Succeeded = $isValid
            FailureMessage = "Expected phone number in format XXX-XXX-XXXX but got '$ActualValue'"
        }
    }
}
...
Invoke-Pester -Path './Tests' -Filter '*Integration*'
```
...
Invoke-Pester -Path './Tests' -CI -OutputFormat 'JUnitXml' -OutputPath './testresults.xml'
```

### Exit Codes
- **0**: All tests passed
- **1**: One or more tests failed
- Use `$LASTEXITCODE` in scripts to check test results

### From Terminal
```powershell
# Run tests and exit with appropriate code
pwsh -NoProfile -Command "Invoke-Pester -Path './Tests' -CI; exit $LASTEXITCODE"
```

---

<!-- End of PowerShell Pester v5 Testing Guidelines Instructions -->
