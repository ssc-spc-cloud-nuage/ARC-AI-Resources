---
id: copilot-docker-instructions
title: Containerization and Docker Best Practices
summary: Guidance for efficient, secure, and reproducible container images and runtimes.
tool: GitHub Copilot
task_tags:
  - docker
  - containerization
...
- **Deeper Dive:**
  ...
  - **Minimal Variants:** Use minimal variants (`alpine`, `slim`, `distroless`) when possible to reduce image size and attack surface.
    - **Alpine Linux:** Preferred for most use cases due to small size (~5MB base), security-focused design with musl libc, and frequent security updates.
    - **Debian Slim:** Alternative for compatibility when Alpine's musl libc causes issues with certain libraries or when glibc is required (e.g., some compiled binaries).
...
### **5. Binary Verification During Build**

- **Principle:** Verify downloaded binaries during build using GPG signatures and SHA256 checksums to ensure authenticity and prevent supply chain attacks.
- **Deeper Dive:**
- **GPG Verification:** Verify cryptographic signatures to ensure binaries come from legitimate sources and haven't been tampered with.
- **Checksum Verification:** Verify SHA256 checksums to detect corruption or tampering during download.
- **Supply Chain Security:** Verification protects against compromised download servers or man-in-the-middle attacks.
- **Build Reproducibility:** Verification failures cause builds to fail immediately, preventing deployment of compromised images.
- **Guidance for Copilot:**
- Always verify signatures and checksums when downloading binaries from external sources during build.
- Use official GPG keys from vendor sources (e.g., HashiCorp, GitHub).
- Verify checksums using official checksum files, not inline checksums that could be compromised.
- Clean up verification artifacts (GPG keys, signature files) in the same layer.
- Recommend using package manager verification features when available.
- **Example (HashiCorp Binary Verification):**

```dockerfile
# Install Terraform with full GPG signature verification
RUN apk add --no-cache unzip gnupg \
    ...
    # Download Terraform and signature files\
    && curl -Lo /tmp/terraform_${VERSION}_linux_amd64.zip https://releases.hashicorp.com/terraform/${VERSION}/terraform_${VERSION}_linux_amd64.zip \
    ...
    && cd /tmp && grep "terraform_${VERSION}_linux_amd64.zip" terraform_SHA256SUMS > terraform_SHA256SUMS.filtered \
    && sha256sum -c terraform_SHA256SUMS.filtered \
    # Extract verified binary\
    && unzip /tmp/terraform_${VERSION}_linux_amd64.zip -d /usr/local/bin/ \
    && chmod +x /usr/local/bin/terraform \
    # Archive GPG key for later verification if needed\
    && mkdir -p /usr/share/hashicorp \
    && cp /tmp/hashicorp.asc /usr/share/hashicorp/hashicorp.asc \
    # Clean up temporary files and GPG keyring in same layer\
...
- **Two-layer verification:** GPG signature proves authenticity, SHA256 checksum proves integrity
- **Fail fast:** Build fails immediately if verification fails, preventing deployment of compromised binaries
- **Official sources:** Use vendor-provided GPG keys and checksum files, not third-party sources
- **API reliability:** Use GitHub API for releases instead of scraping web pages for hash values
- **Cleanup:** Remove verification tools and sensitive data (GPG keyring) in the same layer

**When to Apply:**

- Any binary downloaded from external sources during build
- Especially critical for: security tools, compilers, runtime environments, infrastructure tools
- Required for: compliance, security-sensitive environments, production images

### **6. Limit Capabilities & Read-Only Filesystems**
